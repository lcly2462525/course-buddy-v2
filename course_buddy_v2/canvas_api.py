...
def get_active_courses(token: Optional[str] = None) -> List[Dict]:
    ...
    while url:
        try:
            ...
            raise RuntimeError("连接 Canvas 超时。请检查网络或代理后重试。")
        except requests.exceptions.ConnectionError:
            raise RuntimeError("无法连接 Canvas（oc.sjtu.edu.cn）。请检查网络 / VPN / 代理。")
        if response.status_code == 401:
            raise RuntimeError("Canvas API Token 无效或已过期。请更新 ~/.config/canvas/token。")
